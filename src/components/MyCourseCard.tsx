...
function MyCourseCard({ course, progress }: MyCourseCardProps) {
    ...
    return (
        ...
        <Link to={`/course/${course.id}`}>
            <div className="group w-full h-100 lg:h-105 border rounded-xl overflow-hidden shadow border-border cursor-pointer transition-all ">
                ...
                <div className="w-full h-[50%] p-3 lg:p-5 flex flex-col justify-between gap-2 lg:gap-3 bg-surface text-text">
                    ...
                    <div className="text-sm mb-1">
                        {progress === 100 ? (
                            <p className="text-green-500">✓ เรียนจบแล้ว</p>
                        ) : (
                            <p className="text-muted">กำลังเรียน {progress}%</p>
                        )}
                    </div>
                </div>
            </div>
